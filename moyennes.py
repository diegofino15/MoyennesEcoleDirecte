...
def calculate_averages(averages):
    ...
    trimestres_codes = utilities.get_codes()['trimetres_codes']
    subjects_codes = utilities.get_codes()['subjects_codes']
...
def return_result(parameters, return_type='t'):
    if return_type == 's':
        with open('./index.html', 'w') as file:
            file.write(utilities.print_website(parameters))
            file.close()
        sys.exit()

    elif return_type == 't':
        for i in range(3):
            print(utilities.return_terminal(parameters, i))
        sys.exit()
